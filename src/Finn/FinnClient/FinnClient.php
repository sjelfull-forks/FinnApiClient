    ...
    private function parseEntry ($entry, $ns)
    {
        ...
        if ( $entry->children($ns['media']) && $entry->children($ns['media'])->content->attributes() ) {
            ...
            foreach ($entry->children($ns['media'])->content as $content) {
                $line                = current($content->attributes());
                $line['description'] = current($content->description);
        ...
        foreach ($adata->children($ns['finn'])->field as $field) {
            ...
            if ( $field->attributes()->name == 'construction_year' ) {
                $constructionYear = $field->attributes()->value;
            }

            if ( $field->attributes()->name == 'energy_label' ) {
                $energyLabel = $field->attributes()->value;
            }

            if ( $field->attributes()->name == 'floor' ) {
                $floor = $field->attributes()->value;
            }
            ...
            if ( $field->attributes()->name == 'size' ) {
                foreach ($field->children($ns['finn'])->field as $sizeField) {
                    ...
                    if ( $sizeField->attributes()->name == "gross" ) {
                        $grossSize = $sizeField->attributes()->value;
                    }
        ...
        $property->constructionYear = (string)$constructionYear;
        $property->energyLabel      = (string)$energyLabel;
        ...
        $property->grossSize        = (string)$grossSize;
        ...
        foreach ($adata->children($ns['finn'])->price as $price) {
            if ( $price->attributes()->name == 'main' ) {
                $salesCost = $price->children($ns['finn'])->sales_cost;
        ...
        $property->salesCost      = (string)$salesCost;
